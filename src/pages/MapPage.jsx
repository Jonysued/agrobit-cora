...
export default function MapPage(){
  ...
  const gLast=gradFor(m=>m.last?.kg_ha),gMean5=gradFor(m=>m.mean5),gBest=gradFor(m=>m.best?.kg_ha),gEst=gradFor(m=>m.objective?.estimated_kg_ha),gTrend=gradFor(trendValue);
  const color=(l)=>{const m=lotMetrics(l,d.ProductionRecord,d.Objective,d.HealthRecord);if(view==='Estado sanitario')return m.healthState==='Bueno'?'#16a34a':m.healthState==='Intermedio'?'#eab308':'#dc2626';if(view==='Problemas sanitarios recurrentes'){const n=d.HealthRecord.filter(h=>h.lot_id===l.id).length;return n<2?'#16a34a':n<3?'#eab308':'#dc2626'}if(view==='Cumplimiento del objetivo')return m.compliance>=100?'#16a34a':m.compliance>=85?'#eab308':'#dc2626';if(view==='Tendencia productiva')return gTrend(trendValue(m));const metric=view==='Producción estimada campaña actual'?m.objective?.estimated_kg_ha:view==='Promedio 5 campañas'?m.mean5:view==='Mejor campaña histórica'?m.best?.kg_ha:view==='Objetivo campaña actual'?m.objective?.kg_ha:view==='kg/planta'?(m.last?.kg_plant||0)*700:m.last?.kg_ha;if(!['Cultivo','Variedad','Edad','Año de plantación','Densidad'].includes(view)){if(view==='Producción última campaña')return gLast(metric);if(view==='Promedio 5 campañas')return gMean5(metric);if(view==='Mejor campaña histórica')return gBest(metric);if(view==='Producción estimada campaña actual')return gEst(metric);return metric>=ranges.high?'#16a34a':metric>=ranges.low?'#eab308':'#dc2626';}const key=view==='Variedad'?l.variety:view==='Cultivo'?l.crop:view==='Edad'?new Date().getFullYear()-l.planting_year:view==='Densidad'?10000/(l.row_spacing*l.plant_spacing):l.planting_year;return categorical[Math.abs(String(key).split('').reduce((a,c)=>a+c.charCodeAt(0),0))%categorical.length]};
