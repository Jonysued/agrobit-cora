...
export default function MapPage(){
  ...
  const prodVals=lots.map(l=>{const m=lotMetrics(l,d.ProductionRecord,d.Objective,d.HealthRecord);return m.last?.kg_ha||0}).filter(v=>v>0);
  const prodMin=prodVals.length?Math.min(...prodVals):0,prodMax=prodVals.length?Math.max(...prodVals):0;
  const lerp=(a,b,t)=>Math.round(a+(b-a)*t),toHex=x=>x.toString(16).padStart(2,'0');
  const gradColor=(v)=>{if(!prodMax||prodMax===prodMin)return '#eab308';const t=(v-prodMin)/(prodMax-prodMin);const r=t<=.5?lerp(220,234,t*2):lerp(234,22,(t-.5)*2),g=t<=.5?lerp(38,179,t*2):lerp(179,163,(t-.5)*2),b=t<=.5?lerp(38,8,t*2):lerp(8,74,(t-.5)*2);return `#${toHex(r)}${toHex(g)}${toHex(b)}`};
  const color=(l)=>{const m=lotMetrics(l,d.ProductionRecord,d.Objective,d.HealthRecord);if(view==='Estado sanitario')return m.healthState==='Bueno'?'#16a34a':m.healthState==='Intermedio'?'#eab308':'#dc2626';if(view==='Problemas sanitarios recurrentes'){const n=d.HealthRecord.filter(h=>h.lot_id===l.id).length;return n<2?'#16a34a':n<3?'#eab308':'#dc2626'}if(view==='Cumplimiento del objetivo')return m.compliance>=100?'#16a34a':m.compliance>=85?'#eab308':'#dc2626';if(view==='Tendencia productiva')return m.trend==='Creciente'?'#16a34a':m.trend==='Decreciente'?'#dc2626':'#eab308';const metric=view==='Producción estimada campaña actual'?m.objective?.estimated_kg_ha:view==='Promedio 5 campañas'?m.mean5:view==='Mejor campaña histórica'?m.best?.kg_ha:view==='Objetivo campaña actual'?m.objective?.kg_ha:view==='kg/planta'?(m.last?.kg_plant||0)*700:m.last?.kg_ha;if(!['Cultivo','Variedad','Edad','Año de plantación','Densidad'].includes(view)){if(view==='Producción última campaña')return gradColor(metric);return metric>=ranges.high?'#16a34a':metric>=ranges.low?'#eab308':'#dc2626';}const key=view==='Variedad'?l.variety:view==='Cultivo'?l.crop:view==='Edad'?new Date().getFullYear()-l.planting_year:view==='Densidad'?10000/(l.row_spacing*l.plant_spacing):l.planting_year;return categorical[Math.abs(String(key).split('').reduce((a,c)=>a+c.charCodeAt(0),0))%categorical.length]};
