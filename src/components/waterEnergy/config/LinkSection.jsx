...
// Vinculación de cada perfil de suelo con su sensor de humedad y su bomba.
// La tarifa energética es global (igual para todos los lotes).
export default function LinkSection({ lots, profiles, sensors, pumps, onChange }) {
  const save = async (profile, field, value) => {
    await waterForecastService.saveProfile({ ...profile, [field]: value || null });
    onChange();
  };
  return (
    <ConfigPanel title="Vinculación de perfiles" description="Linkeá cada perfil de suelo a su sensor de humedad y a su bomba — las estimaciones y proyecciones se calculan con esa vinculación. La tarifa energética es global para todos los lotes.">
      {!profiles.length ? (
