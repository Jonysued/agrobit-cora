...
const EMPTY = { name: '', farm: '', energy_supply_number: '', lot_id: '', irrigation_sector: '', power_kw: '', flow_m3_h: '', design_pressure_bar: '', efficiency_percent: '', active: true };
const num = v => (v === '' || v == null ? null : Number(v));

export default function PumpSection({ lots, pumps, onChange }) {
  const [form, setForm] = useState(null);
  const [busy, setBusy] = useState(false);
  const farms = [...new Set(lots.map(l => l.farm).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  ...
  const save = async e => {
    ...
    const payload = {
      ...
      energy_supply_number: form.energy_supply_number?.trim() || null,
  ...
  return (
    <ConfigPanel title="Pozos y bombas" description={`Pozos y bombas con su suministro eléctrico y datos de operación · ${pumps.length} configurado(s)`}>
      {!form && <button onClick={() => setForm({ ...EMPTY })} className="flex items-center gap-1.5 rounded-lg bg-emerald-900 px-3 py-1.5 text-xs font-bold text-white transition hover:bg-emerald-800"><Plus size={14} />Nuevo pozo / bomba</button>}
      {form && (
        <form onSubmit={save} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          <div className="mb-3 flex items-center justify-between">
            <b className="text-sm text-charcoal">{form.id ? 'Editar pozo / bomba' : 'Nuevo pozo / bomba'}</b>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field label="Finca">
              <select value={form.farm || ''} onChange={e => set('farm', e.target.value)} className={inputCls}>
                <option value="">—</option>
                {farms.map(farm => <option key={farm} value={farm}>{farm}</option>)}
              </select>
            </Field>
            <Field label="Número de suministro eléctrico"><input inputMode="numeric" value={form.energy_supply_number || ''} onChange={e => set('energy_supply_number', e.target.value)} className={inputCls} placeholder="20002313312" /></Field>
            ...
            <Field label="Caudal (m³/h)"><input type="number" step="any" min="0" value={form.flow_m3_h ?? ''} onChange={e => set('flow_m3_h', e.target.value)} className={inputCls} placeholder="Pendiente" /></Field>
          ...
          <button disabled={busy} className="mt-4 w-full rounded-xl bg-emerald-900 py-2.5 font-bold text-white transition hover:bg-emerald-800 disabled:opacity-60">{busy ? 'Guardando…' : 'Guardar pozo / bomba'}</button>
        </form>
      )}
      {pumps.length > 0 && (
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[980px] text-sm">
            <thead><tr className="border-b border-slate-200 text-left text-[11px] uppercase tracking-wide text-slate-400"><th className="py-2 pr-3">Nombre</th><th className="pr-3">Finca</th><th className="pr-3">Suministro eléctrico</th><th className="pr-3">Lote / Sector</th><th className="pr-3">Potencia</th><th className="pr-3">Caudal</th><th className="pr-3">kWh/m³</th><th className="pr-3">Estado</th><th /></tr></thead>
            <tbody>
              {pumps.map(p => (
                <tr key={p.id} className="border-b border-slate-100">
                  <td className="py-2 pr-3"><b className="text-slate-700">{p.name}</b></td>
                  <td className="pr-3 text-slate-600">{p.farm || '—'}</td>
                  <td className="pr-3 font-mono text-xs text-slate-600">{p.energy_supply_number || '—'}</td>
